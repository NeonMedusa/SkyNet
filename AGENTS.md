...
- **工具问题日志（`docs/tool-issues.md`，AI 维护）**：使用自己的工具（read/write/edit/bash/grep/find/ls）
  时遇到 **bug / 无效或低价值提示 / 诊断不完善 / 不得不改用其他办法绕过** 的情况，
  **顺手往该文档记一条**（2-5 行：日期+标题、场景、影响、可能修法、状态）——不要为记录中断主任务，
  也不要只存在自己的上下文里（会话一结束就丢了）。人类按需评估改进；已解决条目移到"已解决"节。
  记录规则与示例见文档开头。
