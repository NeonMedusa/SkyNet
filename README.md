...
- **数据库可能发生破坏性更新。** 项目仍在起步阶段，存储对话的数据库结构随时可能变动。
  程序会对**有迁移路径的旧库**自动迁移（迁移前先整库备份为 `<db>.v{旧版本}.bak.db`）；
  但**过旧的库仍不会被打开**（数据原样保留，可手动备份），且迁移本身也可能出错，
  你可能读不到旧对话。**重要会话请自行备份 `skynet.db`**；如需处理无法自动迁移的旧数据，
  建议把库文件与本项目源码一起交给 AI，请它写一个一次性转换脚本（Python 标准库的
  `sqlite3` 最适合）。
...
- **项目仍在实验阶段：数据库结构随时可能变动**——schema 版本不符时：有迁移路径的旧库会提示后自动迁移（先备份），过旧的库由 TUI 弹窗让你选择"重命名旧库并新建"或"退出"，库比程序新则提示升级；CLI 子命令不走弹窗（直接报错）
