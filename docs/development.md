...
| 项 | 来源 | 说明 |
|---|---|---|
| ~~`edit` 支持 `edits[]` 数组~~ | pi 有实现 | **✅ 已完成（2026-09-22）**：改为 `edits[]` 单轨（schema 只暴露 path + edits[]），带容错（edits 传单对象）与全有或全无语义 |
| ~~`grep` 输出按文件分组~~ | opencode 风格 | **✅ 已完成（2026-09-22）**：`Found N matches` 统计 + `path:` 标题行 + `  Line N: text` 行格式 |
