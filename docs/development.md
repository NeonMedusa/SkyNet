...
> **模块职责表、`main.zig` 分区地图、关键数据流（含函数名锚点）见
> [`docs/architecture.md`](architecture.md)**——本节只保留该文档未覆盖的
> 并发/渲染/持久化细节。

### 1.1 两套数据的边界（重要）

显示层 `AppState.messages`（含思考块/工具块等渲染态）与模型历史
`AppState.history`（API 消息格式）是**独立两套**，只在 finalize、排队注入、
落库/恢复处汇合。改任一路径时先想清楚另一路是否需要同步。改代码前先读
[`docs/architecture.md`](architecture.md) 的“改动连带影响”表。
...
- **已合并上游**：PR #38（非法 UTF-8 容错）、#39（Windows 终端还原钩子）、
  #42（宽字符支持：代理对输入 + 组件按显示宽度布局）；
- **本地已有、待移植推送**（fork 的两个独立 commit，属通用光标能力）：
  **pending cursor 帧尾定位**（修 IME 候选窗闪烁）与 **DECSCUSR 光标形状**；
- **后续候选**：bracketed paste（POSIX 接线）、Windows VT 输入（先 issue 探路）、
...
（断连重试的当前参数：2 次、2s/4s ±25%，代码常量 `stream_retry_max` /
`stream_retry_delayMs`；网关长时间抖动时调大即可。）

### 2.4 设计讨论（存档）：会话跨目录、/cd 与提示词系统
...
Claude Code / Cursor / Aider 用 `old_string`/`old_str`，我们用蛇形 `old_text`/`new_text`。

**设想**：容错解析层（`prepareEditArgs`）额外接受其他命名的字段（如 pi 的驼峰），
让"从其他 agent 迁移的对话历史"或"被其他工具 prompt 过的模型"也能直接调用成功。

**为什么是"可讨论"而非"待办"**：
- 实用性有争议：模型主要按**当前**工具描述生成参数（描述里写的是 `old_text`），
  跨 agent 兼容只在"历史上下文里残留了其他工具用法"时才有价值——这种场景少见；
- 成本与风险：每多一种命名就多一份解析分支与测试；命名歧义（同时给了两种）需要额外
  裁决规则；
- 反例：pi 与 opencode **互不兼容**对方字段（两家都没做跨 agent 兼容），说明主流选择
  是"各用各的、不做兼容"。

**结论**：暂不做；若将来出现真实需求（如用户抱怨"换了 agent 后旧习惯失效"）再评估。
当前容错层只保留一种：`edits` 传成单个对象（纯格式差异，无命名歧义）。

### 2.6 生成中手动压缩（排队）——已实现（2026-09-22）

生成中按 `/compact` 不再被拒：**排队到下一个工具轮间隙执行**，与自动压缩
**同位置同函数**（`streamWorker` 循环的 `injectPendingSends` 之后）。用户可见行为：

- 排队提示 `压缩已排队，将在下一个工具轮执行`；重复触发不重复入队（`/compact N`
  的最新保留窗口生效）；间隙压缩正在执行时提示 `压缩正在进行中` 且不入队；
- 本轮未碰间隙（单轮纯回答/被取消）→ `flushPendingSends` 空闲时补执行；
- 无可压内容时不发请求（toast `上次压缩后暂无新增内容，无需压缩`）；
- 不单独加"最小 token 阈值"（`selectCompactionRange` 的 null 判据已足够）。

实现要点：`compact_queued` / `compact_queued_keep` 原子标志；`compactJobHistory`
新增 `force` 与 `keep_override` 参数；worker 用新事件 `.toast` 回传提示。
交互细节与用户说明见 `docs/user-guide.md` 3.8。

**参考：自动压缩**（未变）：工具轮间隙触发；压缩后重建 `job.history`，**AI 继续干
未完成的活**（与 pi 一致）。备选（未采纳）：对齐 pi 的"中止当前响应立刻压缩"——
立即生效但丢失 AI 正在做的活；现可用 `Ctrl+Q` 中止后再 `/compact`。
