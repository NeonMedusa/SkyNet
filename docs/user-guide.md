...
- **自动**：请求达到窗口的 75%（默认）时触发；保留最近约 20000 token 不动。
  两种触发点都以**流式**方式展示摘要（橙色标题 + 可折叠思考块 + 正文）：
  - 新一轮请求前（空闲时）：触发一次同步压缩，摘要就地生成；
  - 工具循环中途（生成中）：与手动 `/compact` 完全一致，边生成边渲染；
    此时输入框标题也显示"压缩中"（与手动压缩统一）。
- **手动**：`/compact`（或菜单里的 compact）随时压缩；`/compact 30000` 指定保留窗口；
  CLI 可加 `--keep-tokens N`。
  - **生成中按 `/compact`** 不中断当前活：**排队到下一个工具轮间隙执行**
    （与自动压缩同一位置）；快速连按只排队一次（提示`压缩已排队，将在下一个工具轮执行`）。
    若本轮到结束都没碰到间隙（单轮纯回答/被取消），轮尾会自动补执行。
  - 没有可压缩内容时不会发请求：空闲时提示"上次压缩后暂无新增内容/内容较少"，
    生成中排队则弹一个短暂提示。
- **摘要的位置**：摘要按它**生成时的位置**显示（与普通消息一样按时间顺序），
  重启后位置不变（不插入到压缩交界处）；被压缩的旧消息仍保留在显示中，可向上滚动回看。
- 摘要生成过程也会显示**思考块**（与普通回复一致：生成中展开、正文开始时折叠、可点击展开）。
- **视觉标识**：输入框边框按状态换色——生成中青色（`cyan`）、**压缩中橙色**
  （`rgb(255,165,0)`，手动与工具轮间隙的自动压缩统一）、空闲品红；
  压缩标题 `▣ …` 与压缩中的边框共用同一橙色强调色。终端不支持 24-bit
  色时降级到近似色。
- 工具输出另有独立的**折叠**机制（较早的大输出替换为一行提示，见 3.11），不需要手动管理。
...
较早的大工具输出（如长文件、大量搜索/命令输出）会从**发给 AI 的内容**中替换为一行
提示（`[工具输出已折叠：bash 输出 12345 字符。如需内容请重新调用该工具]`），
以控制上下文规模；AI 可重新调用工具拿回内容。

**你的数据和界面不受影响**：数据库始终保存全文（导出/查看均为完整内容），
聊天区也始终显示全文，不会看到折叠提示。
