...
const isFoldableMessage = context_mod.isFoldableMessage;
const FoldSim = context_mod.FoldSim;
const simulateFold = context_mod.simulateFold;
...
const AppState = struct {
    ...
    /// 估算当前请求 token（历史内容 + tool_calls 参数 + 工具 schema；chars/4）
    ...
    fn finalizeStream(self: *AppState, err: ?ai.AIError) void {
        ...
        if (self.stream_job) |job| {
            ...
            if (compacted_midturn) self.rebuildHistoryFromDb();
        }
    ...
    /// 返回折叠条数（0 = 未触发）。只处理已落库（db_id != 0）的结果，保证重启后前缀一致。
    fn maybeFoldOldToolOutputs(self: *AppState) usize {
        ...
        while (i > 0) {
            ...
            if (!isFoldableMessage(m)) continue;
            if (protected_bytes < fold_protect_bytes) {
                protected_bytes += m.content.len;
            } else {
                candidate_bytes += m.content.len;
            }
        }
        if (candidate_bytes < fold_min_bytes) return 0;

        // 第二遍：批量折叠（保留最近窗口不动）
        var protected_again: usize = 0;
        var folded: usize = 0;
        var saved: usize = 0;
        ...
        while (i > 0) {
            ...
            const m = &self.history.items[i];
            if (!isFoldableMessage(m.*)) continue;
            if (protected_again < fold_protect_bytes) {
                protected_again += m.content.len;
                continue;
            }
            const tool_name = blk: {
                for (calls.items) |c| {
...
fn cmdStats(allocator: Allocator, io: Io, opt: CliOptions) u8 {
    ...
    var tool_contents = std.ArrayListUnmanaged([]const u8){ .items = &.{}, .capacity = 0 };
    defer tool_contents.deinit(allocator);
    ...
    for (active) |r| {
        ...
        tool_contents.append(allocator, r.content) catch {};
    ...
    const sim = simulateFold(tool_contents.items);
...
test "旧工具输出折叠：批量触发、全文保留、重启后请求体一致" {
    ...
    try std.testing.expectEqual(@as(usize, 6), h.len);
...
test "独立验证：重启后块工具用全文重建、大小统计按原文、多轮折叠前缀稳定" {
    ...
    // 回合 2..5：再 4 条 read，把最旧两条推出保护窗口
    for (2..6) |i| {
        try verifyAppendToolRound(alloc, &live, i, "read", "{}", read_buf);
    }

    // 第一次折叠：保护最近 160KB → 最旧 2 条（read、bash）折叠
    ...
    // 多轮：追加一条 200KB 输出再折叠，应只折新的 4 条候选，旧 stub 逐字节不变
    const big_buf = try alloc.alloc(u8, 200_000);
    defer alloc.free(big_buf);
    @memset(big_buf, 'z');
    try verifyAppendToolRound(alloc, &live, 6, "read", "{}", big_buf);
    try std.testing.expectEqual(@as(usize, 4), live.maybeFoldOldToolOutputs());
    for (live.history.items) |m| {
